...
var colors = ['#00bdff', '#4d39ce', '#088eff']; // Functions
...
function () {
  function particle(x, y, radius, color) {
    ...
    this.y = y;
    this.radius = radius;
    this.color = color; // to make sure the spawn is different, get a random value anywhere from 0 to Pi*2 -> which is the entire travel of a circle
    ...
    this.draw = function (lastPoint) {
      ...
      c.lineWidth = _this.radius; // coordinates for particle previous frame (I had to create it in the update function)

      c.moveTo(lastPoint.x, lastPoint.y); // coordinates for particle next frame / new location
...
function animate() {
  requestAnimationFrame(animate); // for each frame we are drawing a new white rectangular on top of it with trasparency, as we lay more on top of each other we start having this effect

  c.fillStyle = 'rgba(255,255,255,0.1)';
  c.fillRect(0, 0, canvas.width, canvas.height); // need to have a look at the options available
